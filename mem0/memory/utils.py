...
def remove_code_blocks(content: str) -> str:
    ...
    说明：
        该函数使用正则表达式模式来匹配代码块，这些代码块可能以  开头，后跟一个可选的语言标签（字母或数字），并以  结尾。
        如果检测到代码块，它仅返回内部内容，剥离掉标记。
        如果未找到代码块标记，则原样返回原始内容。
    """
    pattern = r"^```[a-zA-Z0-9]*\n([\s\S]*?)\n```$"
...
def parse_vision_messages(messages, llm=None, vision_details="auto"):
    ...
    for msg in messages:
        ...
        if isinstance(msg["content"], list):
            # 情况 A: 内容是一个列表 (通常包含文本 + 多个图片，或者混合内容)
            # 调用 get_image_description 把整个列表里的图片都看懂，合成一段文字描述
            description = get_image_description(msg, llm, vision_details)
            # 将原消息替换为：{"role": "user", "content": "图片里有一只猫在沙发上..."}
            returned_messages.append({"role": msg["role"], "content": description})
        elif isinstance(msg["content"], dict) and msg["content"].get("type") == "image_url":
            # 情况 B: 内容是一个字典，且明确标记为图片 URL (OpenAI 标准格式)
